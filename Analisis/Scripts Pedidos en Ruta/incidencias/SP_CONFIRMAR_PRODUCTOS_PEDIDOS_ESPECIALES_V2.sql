...
--GO
/****** Object:  StoredProcedure [dbo].[SP_CONFIRMAR_PRODUCTOS_PEDIDOS_ESPECIALES_V2]    Script Date: 06/03/2022 01:47:48 p. m. ******/
SET ANSI_NULLS ON
GO
...
ALTER proc [dbo].[SP_CONFIRMAR_PRODUCTOS_PEDIDOS_ESPECIALES_V2]
		...
		begin try
			...
			begin --declaraciones 
				...
				declare @status						int = 200,
						...
						@idFactFormaPagoAbono	    int = 0,
						@idTipoTicketPedidoEspecial int = cast(1 as int)
			...
			begin -- principal
				...
				select @esPedidoEnRuta = coalesce(@esPedidoEnRuta, cast(0 as bit))
				...
				-- 9	Pedido en Ruta
				if  ( 
						( @idEstatusPedidoEspecial in (4,6) ) or 
						( @esPedidoEnRuta = cast(1 as bit) and @idEstatusPedidoEspecial = 9 )   -- si es liquidacion de un pedido en ruta
					)
					begin
				...
				if ( ( @aCredito = cast(1 as bit) ) or ( @aCreditoConAbono = cast(1 as bit) ) )
					begin	
					    select @idFactFormaPagoAbono=@idFactFormaPago;
				...
				set		idEstatusPedidoEspecial = @idEstatusPedidoEspecial,
						...
						idUsuarioLiquida = @idUsuarioLiquida
				where	idPedidoEspecial = @idPedidoEspecial
				...
				-- se revisa si hay rechazos o pedidos no aceptados para hacer las devoluciones correspondientes
				...
				if ( @hayRechazos = cast(1 as bit) or @hayNoAceptados = cast(1 as bit) )
					begin
						...
						from	#productos p
									left join Ubicacion u
										on p.idAlmacenDestino = u.idAlmacen
						...
						from	#tempUbicacionesDevoluciones_ temp
									join (	
											select	p.idProducto, p.idAlmacenDestino,  (sum(p.cantidadAtendida) - sum(cantidadAceptada)) as noAceptados
											from	#productos p
											where	(p.cantidadAtendida - cantidadAceptada)	> 0
											group by p.idProducto, p.idAlmacenDestino
										 )rechazados on rechazados.idProducto = temp.idProducto 
						...
						from	(
									...
									from	#tempUbicacionesDevoluciones_ temp
												join (
														select	p.idProducto, p.idAlmacenDestino, sum(p.cantidadRechazada) as cantidadRechazada												
														from	#productos p
														where	(p.cantidadAtendida - cantidadAceptada) > 0
														group by p.idProducto, p.idAlmacenDestino
													 )rechazados on rechazados.idProducto = temp.idProducto
																and	rechazados.idAlmacenDestino = temp.idAlmacenDestino
												join Ubicacion u
													on	u.idAlmacen = temp.idAlmacenDestino
						...
						from	(
									...
									from	#tempUbicacionesDevoluciones_ temp
												join (
														select	p.idProducto, p.idAlmacenDestino, sum( (p.cantidadAtendida - cantidadAceptada)) as noAceptados												
														from	#productos p
														where	(p.cantidadAtendida - cantidadAceptada) > 0
														group by p.idProducto, p.idAlmacenDestino
													 )rechazados on rechazados.idProducto = temp.idProducto
					...
					if	( 
							( @idEstatusPedidoEspecial <> 9 )  or
							( ( @esPedidoEnRuta = cast(1 as bit) and @idEstatusPedidoEspecial = 9 ) )  -- si es liquidacion de pedido especial en ruta 
						)
						begin
							...
							-- se actualizan las cantidades aceptadas en el inventario detalle 
							update	InventarioDetalle 
							set		cantidad -= a.cantidadAceptada, 
					...
					if ( ((@aCredito = cast(1 as bit)) or (@aCreditoConAbono = cast(1 as bit)) ) and (@idEstatusPedidoEspecial not in (9)) )
						begin
					...
					if ( (@montoPagado > 0.0) and ( ( @aCredito = cast(1 as bit) ) or ( @aCreditoConAbono = cast(1 as bit ) ) ))
						begin
							...
							select	@idUsuario as idUsuario, @montoPagado as monto, @montoIva as montoIva, @montoComision as montoComision,
									...
									@idFactFormaPagoAbono as idFactFormaPago, @idFactUsoCFDI as idFactUsoCFDI, @fecha as fechaAlta, cast(1 as bit) as activo
