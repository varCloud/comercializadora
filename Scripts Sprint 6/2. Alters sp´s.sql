...
GO
/****** Object:  StoredProcedure [dbo].[SP_CONSULTA_PRODUCTOS]    Script Date: 09/09/2020 07:51:47 p. m. ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
					...
					begin
						...
						from	(
									select	id.idProducto, 
											coalesce( (sum(id.cantidad)), 0) as cantidad,
											case
												when id.cantidad is null then upper(p.descripcion) + '  - (S/E)'
												when id.cantidad = 0  then upper(p.descripcion) + '  - (S/E)'
												else upper(p.descripcion) + 
														'  - (E:'  + cast(id.cantidad as varchar(500)) + ' / ' +
															 'ME:' + cast(isnull(p.precioIndividual,'') as varchar(500)) + ' / ' +
															 'MA:' + cast(isnull(p.precioMenudeo,'') as varchar(500)) +
															 ')'
											end as descripcionConExistencias
									from	Usuarios u
												...
												inner join Productos p
													on p.idProducto = id.idProducto
									where	u.idUsuario = @idUsuario
									group by id.idProducto, p.descripcion, id.cantidad, p.precioIndividual, p.precioMenudeo
								)A
						where	#Productos.idProducto = a.idProducto
